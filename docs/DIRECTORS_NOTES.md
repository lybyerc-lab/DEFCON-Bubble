...
- Foundation, typed damage, the accepted soap-film POP, castle impact, First Defense, the mixed three-wave encounter, and the first upgrade choice are promoted on `main`.
- Treat accepted POP, castle consequence, enemy identities, mixed-encounter timing, and the upgrade seam as reusable contracts. Reopen them only for new player evidence, a measured defect, or production-scale requirements.
...
- Visual quality is now a named milestone rather than a someday concern. The current question is `[DB:PRESENTATION:LIT_BEACH]`: light the scene and make bubbles behave like soap film. The scene has never had a `WorldEnvironment`, and the enemies have been unshaded since they were built, so this is closing an omission rather than adding polish.
- Light and material first, geometry later. A lit scene changes how every later art decision reads, so sand, water, camera, and any sculpted geometry should be judged on top of it rather than before it.
- Iridescence is identity, not decoration, but it has a taste ceiling. Bubbles should look like soap film catching the sun, not like a rainbow. If the family stops reading as one faceless species at phone size and speed, the effect has gone too far.
- Brightening the world puts HUD contrast and glow bleed at risk. Readability of wave, castle, terminal, and upgrade messaging is part of the visual gate, not a separate concern.
...
- The Drive vertical-slice plan's gameplay sequencing remains a useful reference. Its original desktop-first prototype wording is superseded by the repository's mobile-first decision.
- Environment presentation has its own reusable scene boundary. Grow the beach through composed pieces without letting scenery own encounter or upgrade truth.
