...
The current bounded milestone is `[DB:ENCOUNTER:MIXED_THREE_WAVE]`: present inherited BasicBubble enemies as faceless, elongated bipedal monsters whose horns and limbs are made from smaller translucent bubbles; introduce the Fast variant; and run one authored three-wave Basic/Fast encounter against the same persistent two-health CastleChunk. The mixed range places that castle at x=-4.4 and parents the toothpick origin to its roof. Creature appendages and locomotion poses are presentation-only. Small typed spawn/wave Resources describe only the exact schedule; MixedEncounter owns READY/WAVE_ACTIVE/INTERMISSION/WON/LOST progression.
